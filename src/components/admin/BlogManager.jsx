...
const BlogManager = () => {
  ...
  useEffect(() => {
    const transl = editingPost?.translations || {}
    setTranslations(transl)
    if (editingPost?.content) {
      setBlocks(deserializeHtmlToBlocks(editingPost.content, transl))
    } else if (!editingPost) {
      setBlocks([])
    }
  }, [editingPost])
  ...
  const handleSubmit = async (e) => {
    ...
    const validation = validatePageBeforePublish({ type: 'blog', translations }, contentHtml)
    if (!validation.valid) {
        ...
        >
          ...
          {showPreview && (
            <div className="mt-3 bg-secondary/50 p-4 rounded-xl">
              ...
              <p className="text-xs text-muted-foreground mt-2">SEO تلقائي: {(translations[activeLocale]?.title || 'العنوان') + ' | Grantify Academy'} | {(translations[activeLocale]?.excerpt || 'الوصف التلقائي')}</p>
            </div>
          )}
            <div>
