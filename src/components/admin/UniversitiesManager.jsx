...
import { getUniversities, createUniversity, updateUniversity, deleteUniversity, uploadImage, getDrafts, saveDraft, deleteDraft } from '@/lib/storage';
...
const UniversitiesManager = () => {
  ...
  const [programsTranslations, setProgramsTranslations] = useState({});
  ...
  useEffect(() => {
    ...
    const id = setInterval(async () => {
      const formEl = document.querySelector('#university-form')
      if (!formEl) return
      const fd = new FormData(formEl)
      const draft = {
        id: editingUniversity?.id,
        students: fd.get('students'),
        image: imageUrl || editingUniversity?.image || '',
  ...
  }, [showForm, blocks, imageUrl, editingUniversity])
  ...
  useEffect(() => {
    ...
    setProgramsTranslations(editingUniversity?.programsTranslations || {})
  }, [editingUniversity])
  ...
  const handleSubmit = async (e) => {
    ...
    const payload = {
      ...
      location: tBase.location || editingUniversity?.location || '',
      students: formData.get('students'),
      image: imageUrl || editingUniversity?.image || '',
      description: tBase.description || editingUniversity?.description || '',
      programs: (programsTranslations[baseLoc] || editingUniversity?.programs || []),
      content: contentHtml,
      seoTitle: tBase.seoTitle || editingUniversity?.seoTitle || '',
      seoDescription: tBase.seoDescription || editingUniversity?.seoDescription || '',
      translations,
      programsTranslations,
    }
        ...
        >
          ...
          <form id="university-form" onSubmit={handleSubmit} className="space-y-4">
            <div>
              ...
              <Input id="students" name="students" defaultValue={editingUniversity?.students} required />
            </div>
            <div>
              ...
              >
                {uploading ? (
                  ...
                  <>
                    {(imageUrl || editingUniversity?.image) ? (
                      <img src={imageUrl || editingUniversity?.image} alt="preview" className="mx-auto max-h-40 rounded-md" />
                    ) : (
              ...
              {locales.map((loc) => (
                <TabsContent key={loc} value={loc}>
                  <div className="flex justify-end mb-2">
                    <Button type="button" size="sm" variant="outline" onClick={() => { setLocales((prev)=>prev.filter((x)=>x!==loc)); const nt={...translations}; delete nt[loc]; setTranslations(nt); const np={...programsTranslations}; delete np[loc]; setProgramsTranslations(np); }}>
                      حذف هذه اللغة
                    </Button>
                  </div>
                    ...
                    <div>
                      <Label>اسم الجامعة ({loc})</Label>
                        <Input
                          ...
                          onBlur={async (e) => { const draft = { id: editingUniversity?.id, translations: { ...translations, [loc]: { ...(translations[loc]||{}), name: e.target.value } } }; await saveDraft('university', draft); }}
                          placeholder={`اسم (${loc})`}
                        />
                      </div>
                      <div>
                        <Label>الموقع ({loc})</Label>
                        <Input
                          value={translations[loc]?.location || ''}
                          onChange={(e) => setTranslations((prev) => ({ ...prev, [loc]: { ...(prev[loc]||{}), location: e.target.value } }))}
                          onBlur={async (e) => { const draft = { id: editingUniversity?.id, translations: { ...translations, [loc]: { ...(translations[loc]||{}), location: e.target.value } } }; await saveDraft('university', draft); }}
                          placeholder={`الموقع (${loc})`}
                        />
                      </div>
                      <div>
                        ...
                        <Input
                          ...
                          onBlur={async (e) => { const draft = { id: editingUniversity?.id, translations: { ...translations, [loc]: { ...(translations[loc]||{}), seoTitle: e.target.value } } }; await saveDraft('university', draft); }}
                      ...
                      <div className="md:col-span-2">
                        ...
                        <textarea
                          ...
                          onBlur={async (e) => { const draft = { id: editingUniversity?.id, translations: { ...translations, [loc]: { ...(translations[loc]||{}), description: e.target.value } } }; await saveDraft('university', draft); }}
                      ...
                      <div className="md:col-span-2">
                        ...
                        <Input
                          ...
                          onBlur={async (e) => { const draft = { id: editingUniversity?.id, translations: { ...translations, [loc]: { ...(translations[loc]||{}), seoDescription: e.target.value } } }; await saveDraft('university', draft); }}
                          placeholder={`SEO Description (${loc})`}
                        />
                      </div>
                      <div className="md:col-span-2">
                        <Label>البرامج الدراسية ({loc})</Label>
                        <textarea
                          className="w-full min-h-[100px] rounded-md border bg-transparent px-3 py-2"
                          value={(programsTranslations[loc] || []).join('\n')}
                          onChange={(e) => setProgramsTranslations((prev) => ({ ...prev, [loc]: e.target.value.split('\n').map((x) => x.trim()).filter(Boolean) }))}
                          onBlur={async (e) => { const draft = { id: editingUniversity?.id, programsTranslations: { ...programsTranslations, [loc]: e.target.value.split('\n').map((x) => x.trim()).filter(Boolean) } }; await saveDraft('university', draft); }}
                          placeholder={`كل برنامج في سطر (${loc})`}
                        />
                      </div>
                    </div>
                  </TabsContent>
          ...
          {showPreview && (
            <div className="mt-3 bg-secondary/50 p-4 rounded-xl">
              ...
              <div className="mt-2">
                <p className="text-sm font-semibold">البرامج:</p>
                <ul className="list-disc pr-4 text-muted-foreground">
                  {(programsTranslations[activeLocale]||[]).map((p,i)=>(<li key={i}>{p}</li>))}
                </ul>
              </div>
            </div>
          )}
                      ...
                      <div>
                        ...
                        <div className="mt-2 flex gap-3 overflow-x-auto scrollbar-hide">
                          ...
                          <div
                            ...
                            onClick={() => fileInputRef.current?.click()}
                            onDragOver={(e) => e.preventDefault()}
                            onDrop={async (e) => { e.preventDefault(); const file = e.dataTransfer.files?.[0]; if (file) { setUploading(true); const url = await uploadImage(file, 'universities'); setUploading(false); setBlocks((prev) => prev.map((x,i) => i===idx ? { ...x, images: [...(x.images||[]), url] } : x)); toast({ title: 'تم رفع الصورة' }); } }}
                          >
                            {uploading ? 'جارٍ الرفع...' : 'أضف صورة'}
                          </div>
                          <input ref={fileInputRef} type="file" accept="image/*" className="hidden" onChange={async (e) => { const f=e.target.files?.[0]; if (f){ setUploading(true); const url=await uploadImage(f,'universities'); setUploading(false); setBlocks((prev) => prev.map((x,i) => i===idx ? { ...x, images: [...(x.images||[]), url] } : x)); toast({ title: 'تم رفع الصورة' }); } }} />
                        </div>
                      </div>
                    )}
                  </div>
        ...
        {universities.map((university, index) => (
          ...
          >
            ...
            <div className="p-4">
              ...
              <p className="text-muted-foreground text-sm mb-4">{university.location}</p>
              <div className="flex gap-2">
                <Button
                  size="sm"
                  variant="outline"
